...
import github.awsomefox.audiosonic.adapter.MainAdapter;
...
import github.awsomefox.audiosonic.util.EnvironmentVariables;
...
import github.awsomefox.audiosonic.view.ChangeLog;
	...
	private void showAboutDialog() {
		new LoadingTask<Void>(context) {
			...
			protected void done(Void result) {
				...
				details.add("Anthony Anderson");

				headers.add(R.string.details_email);
				details.add("tonyanderson526@gmail.com");

                headers.add(R.string.details_fork);
                details.add("AudioSonic");
		...
		try {
			...
			new LoadingTask<String>(context) {
				...
				protected String doInBackground() throws Throwable {
					...
					File logcat = new File(Environment.getExternalStorageDirectory(), "audiosonic-logcat.txt");
					Util.delete(logcat);
					...
					try {
						...
						File stacktrace = new File(Environment.getExternalStorageDirectory(), "audiosonic-stacktrace.txt");
