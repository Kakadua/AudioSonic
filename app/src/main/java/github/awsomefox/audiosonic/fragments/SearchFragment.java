...
import github.awsomefox.audiosonic.adapter.SearchAdapter;
...
import github.awsomefox.audiosonic.util.BackgroundTask;
import github.awsomefox.audiosonic.util.TabBackgroundTask;
import github.awsomefox.audiosonic.R;
import github.awsomefox.audiosonic.adapter.ArtistAdapter;
...
public class SearchFragment extends SubsonicFragment implements SectionAdapter.OnItemClickedListener<Serializable> {
	...
	private void autoplay(String query, String artistQuery, String albumQuery, String titleQuery) {
		...
		if(titleQuery != null && !searchResult.getSongs().isEmpty()) {
			...
			TreeMap<Integer, MusicDirectory.Entry> tree = new TreeMap<>();
			for(MusicDirectory.Entry song: searchResult.getSongs()) {
				tree.put(Util.getStringDistance(song.getTitle().toLowerCase(), titleQuery), song);
			}

			Map.Entry<Integer, MusicDirectory.Entry> entry = tree.firstEntry();
